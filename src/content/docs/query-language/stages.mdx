...
Multiple conditions in a single `where` are combined with `AND` or `OR`. Parentheses are supported for explicit grouping, up to 32 nested groups. Multiple `| where` clauses are always AND-ed together. Boolean expressions remain inside the authenticated project's tenant scope; `OR` cannot widen a query to other projects.
...
## Choice-specific stages

```text
| results <conversion_event>
| window <duration>
| unit <user_id|device_id|session_id>
```

`results` returns a machine-readable choice packet with variants, comparisons,
diagnostics, warnings, decision status, method, and generation time. `window`
sets the conversion attribution window. `unit` filters exposures to the
assignment unit used by `choice()`.
