...
Multiple conditions in a single `where` are combined with `AND` or `OR`. Parentheses are supported for explicit grouping, up to 32 nested groups. Multiple `| where` clauses are always AND-ed together.
