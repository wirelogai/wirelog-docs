...
| Latest value | `\| latest event_properties.theme` | One latest property value per entity |
...
<Aside type="tip" title="Script-tag-first projects">
If you only installed the Script Tag, start with `page_view` queries and session attribution queries:
...
# Latest event-derived state by user
themeSwitch | latest event_properties.theme | count by last_value | top 10

# Preferred current user state when written with identify
...
For aggregate-style rows in JSON, including `latest_aggregate`, use:
...
JSON responses report ClickHouse progress in `rows_scanned`. It counts source rows read while executing the query, not result rows returned.
