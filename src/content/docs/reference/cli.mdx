...
Use `wl dashboard sync --file dashboard.yaml` to store a validated version in the API credential's project and render it inline on the authenticated project page. Add root `id`, use `--visibility personal|project`, and use the global `--host` flag for self-hosting. Sync creates no standalone or public URL.
