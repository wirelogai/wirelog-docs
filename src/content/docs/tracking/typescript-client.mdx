...
Every `track()` call in the browser auto-injects `device_id`, `session_id`, and `user_id`. You don't need to pass them.
...
- The Script Tag handles automatic `page_view` tracking, batching, and `sendBeacon` delivery
- The TypeScript client handles typed, explicit event tracking in your React/Vue/Svelte app
...
Track up to 2000 events in one request. In browsers, auto-injects identity per event.
