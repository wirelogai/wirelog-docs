...
| `RateLimit` | `wirelog.RateLimitConfig` | Backend profile | Per-instance burst, sustained, and payload limits; see below |

The Go SDK assumes one process may aggregate an application's backend traffic.
Its zero-value `RateLimit` defaults match WireLog's standard project ingestion
envelope:
...
| Events/minute | `10000` |
| Events/hour | `500000` |
| Events/rolling 24h | `10000000` |
| Event payload | `65536` bytes |

Configure smaller values for a narrowly scoped producer. Set
`RateLimit.Disabled: true` only when another trusted layer provides flood and
payload protection.
...
- Rate-limit and payload rejections are reported to `OnError`; inspect cumulative rate-limit drops with `RateLimitStats()`
- Queue overflow: newest events are dropped; `ErrQueueFull` is reported to `OnError`
- The background worker recovers from panics

`Track()` is deliberately non-blocking and memory-bounded, so it does not offer
durable delivery. Applications that cannot tolerate telemetry loss must monitor
`OnError` outside WireLog and retain the source-of-truth record separately.
...
| `wirelog.ErrRateLimited` | Event dropped by a configured burst or sustained limit |
| `wirelog.ErrPayloadTooLarge` | Event dropped because it exceeded `MaxEventBytes` |
